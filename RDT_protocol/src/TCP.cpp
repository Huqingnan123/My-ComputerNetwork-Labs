...
int main(int argc, char* argv[])
{
	...
	pns->setOutputFile("E:\\vs2019\\Computer-network\\RDT_protocol\\output_TCP.txt");
	pns->start();
	delete ps;
	delete pr;      //指向唯一的工具类实例，只在main函数结束前delete
	delete pUtils;  //指向唯一的模拟网络环境类实例，只在main函数结束前delete
	delete pns;
	return 0;
};